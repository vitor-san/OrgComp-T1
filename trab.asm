.data	# diretiva para inicio do segmento de dados
	...
	str_line: .asciiz "\n"		# quebra de linha
	str_tab: .asciiz "\t"		# tab
	str_sqrt_num: .asciiz "Digite um n�mero inteiro positivo:\n"
	str_sqrt_erro: .asciiz "Entrada inv�lida."
...
main:	# inicio do programa
	...
	bne $t0, $t1, divin		# se o valor que o usuario digitou foi 3, entra na operacao de divisao
		...
		la $a0, str_n1		#salva o endere�o da string em a0
		...
		li $v0, 4		# carrega 4 para v0, (imprimir string)
		la $a0, str_n2		#salva o endere�o da string em a0
		syscall			#printa string
		...
		beqz $a1 divi_erro	# verifica se a entrada � valida
	...
	bne $t0, $t1, sqrtn 		# se o valor que o usuario digitou foi 5, entra na operacao de raiz
		...
		move $a0, $v0 		# move o numero para a0, pois a0 � o parametro da funcao
		
		bltz $a0 sqrtn_erro	# verifica se a entrada � valida
...
subt:
	...
	li $v0, 1		# print_int
	move $a0, $t0		# ira colocar o resultado da subtra��o em $a0, para que seja printado
	syscall
...
erro_n1_mult:
	...
	# que imprime os valores m�ximos de 'a' e 'b' aceitos para uma multiplcacao
	# at� que o resultado (c) de erro (overflow)
...
divi:
	#OBS f0 nao � salvo pois ele sera usado como retorno, sendo assim, seu valor sera alterado de 
	#qualquer forma
	
	subi $sp, $sp, 4 # Move o ponteiro da pilha para o proximo end vazio
	swc1 $f1, ($sp) # Guarda o registrador na pilha, f1 � o denominador da divisao
	
	subi $sp, $sp, 4 # Move o ponteiro da pilha para o proximo end vazio
	swc1 $f2, ($sp) # Guarda o registrador na pilha, f2 � o divisor da divisao
	
	subi $sp, $sp, 4 # Move o ponteiro da pilha para o proximo end vazio
	sw $v0, ($sp) # Guarda o registrador na pilha, v0 � usado para chamar procedimentos
	
	subi $sp, $sp, 4 # Move o ponteiro da pilha para o proximo end vazio
	sw $a0, ($sp) # Guarda o registrador na pilha, a0 � usado para passar parametros para o sistema
	
	subi $sp, $sp, 4 # Move o ponteiro da pilha para o proximo end vazio
	sw $a1, ($sp) # Guarda o registrador na pilha, t0 � usado para armazenar temporariamente o denominador
	...
	mov.s $f0, $f1 # move o f1, q � o x1, para f0 que sera o registrado de retorno
...
sqrt:
	...
	subi $sp, $sp, 4 # Move o ponteiro da pilha para o proximo end vazio
	sw $ra, ($sp) # Guarda o registrador na pilha, ra � o local de retorno na main`
	
	subi $sp, $sp, 4 # Move o ponteiro da pilha para o proximo end vazio
	swc1 $f1, ($sp) # Guarda o registrador na pilha, f1 � a variavel x1, x1 armazena o chute inicialmente
	
	subi $sp, $sp, 4 # Move o ponteiro da pilha para o proximo end vazio
	swc1 $f2, ($sp) # Guarda o registrador na pilha, f2 � a variavel x2, guarda o resultado do calculo
...
fib_seg:
	...
	loop_seg : 
		...
		jal fibonachi #funcao que calcula o fibonachi de a2
